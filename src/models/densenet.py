"""
Adapted from <https://github.com/gpleiss/efficient_densenet_pytorch/blob/
...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features):
        ...
        self.add_module("conv", nn.Conv2d(num_input_features,
                                          num_output_features, kernel_size=1,
                                          stride=1, bias=False))
...
class DenseNet(nn.Module):
    ...
    Args:
        ...
        block_config (list of 3 or 4 ints) - how many layers in each pooling
        block
    ...
    def __init__(self, params, growth_rate=12, block_config=(16, 16, 16),
                 compression=0.5, num_init_features=24, bn_size=4):

        super(DenseNet, self).__init__()
        assert 0 < compression <= 1, "compression of densenet should be "
        "between 0 and 1"
        ...
        # efficient (bool) - set to True to use checkpointing. Much more
        # memory efficient, but slower.
        self.efficient = params.efficient
        ...
        if self.ptc_sz <= 32:
            ...
            features = nn.Sequential(OrderedDict([
                ("conv0", nn.Conv2d(self.ptc_fm, num_init_features,
                                    kernel_size=3, stride=1, padding=1,
                                    bias=False)),
            ]))
        else:
            ...
            features.add_module("pool0", nn.MaxPool2d(kernel_size=3, stride=2,
                                                      padding=1,
                                                      ceil_mode=False))
        ...
        if self.drop_rate > 0:
            features.add_module("dropout_final", nn.Dropout(self.drop_rate))
        features.add_module("adaptavgpool_final", nn.AdaptiveAvgPool2d((1, 1)))
...
def _densenet(params, **kwargs):
    model = DenseNet(params, **kwargs)
    return model


def densenet40(params):
    growth_rate = 12
    depth = 40
    block_config = [(depth - 4) // 6 for _ in range(3)]
    return _densenet(params=params, growth_rate=growth_rate,
                     block_config=block_config)


def densenet100(params):
    growth_rate = 12
    depth = 100
    block_config = [(depth - 4) // 6 for _ in range(3)]
    return _densenet(params=params, growth_rate=growth_rate,
